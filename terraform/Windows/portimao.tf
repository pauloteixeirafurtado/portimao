...
resource "aws_instance" "winsql_portimao_pt" {
  ...
  user_data = data.template_file.winsql-Portimao-pt.rendered
}

resource "aws_eip" "win_Portimao_public_ip" {
  vpc                       = true
  instance                  = aws_instance.winsql_portimao_pt.id
  tags                                 = {
    "Name" = "Portimao public IP"
  }
  depends_on = [
    aws_instance.winsql_portimao_pt
  ]
}